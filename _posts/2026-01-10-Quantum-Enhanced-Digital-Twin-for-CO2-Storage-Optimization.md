...
At each time step k, the system state is characterized by an estimated state:

<div style="text-align:center;">
x̂<sub>k</sub> = [P̂<sub>k</sub>, Ŝ<sub>k</sub>]
</div>

obtained via the Kalman Filter. Based on this state estimate, the objective is to determine an optimal injection rate \( u_k \) that satisfies multiple competing goals:

- **Maximize CO₂ storage**, i.e., increase saturation \( S \)  
- **Minimize pressure buildup** to avoid caprock failure and ensure operational safety  
- **Respect operational constraints** on allowable injection rates  
...
where the objective function encodes trade-offs between storage efficiency and pressure safety. The optimization is performed in a receding horizon manner, where decisions are updated at each time step using the latest estimated state.

Due to the combinatorial nature of discretized control actions and nonlinear dependencies on system state, the problem is reformulated into a Quadratic Unconstrained Binary Optimization (QUBO) problem, enabling the use of quantum optimization techniques.
---
